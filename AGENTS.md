...
menu bar item or on the pager window, or pasted in the pager window), re-encoded
to JPEG ≤ 600 KB and stored inline in the same node (`type: "img"`).
...
- **Where you drop decides whether it sends.** A drop on the *menu bar item* is "share this now" — edit + commit in one step, no window. A drop on the *pager window* becomes the **draft**, exactly like ⌘V, because the window is the editing surface. Both resolve the payload through `DropPayloadClassifier`; only the pasteboard reading differs (`PasteboardPayload` for menu bar/paste, `NSItemProvider` for the window).
- **The drop prompt is an overlay, never a layout change.** The device's height follows its content, and the window is content-sized — so swapping content for a "drop image…" row mid-drag would resize the pager and slide the drop target out from under the cursor. `PagerDeviceState.dropTarget` paints over the screen instead; `design-preview --state drop`/`drop-long` exist to be diffed against `empty`/`long` for exactly this.
- **Opening a pager arms a 10 s "fresh edit" window.** Typing into a just-opened pager starts a new message instead of appending — the UX a select-all would give, without a selection wash on the LCD. `LinkViewModel`'s key monitor sees the key press first: a typing key (`TypingKey.isInsertion`) triggers `EditorSession.takeFreshEdit()`, anything else (arrows, delete, escape) or a click in the window ends the window via `cancelReplaceOnType()`, as does the timeout. What the wipe threw away — text *or* image — stays restorable via ⌘Z (`restoreReplaced`, also intercepted in the monitor, because the field editor's own undo stack knows nothing about it) until the draft is sent, cleared or discarded.
...
- **`Playout/`** — `PlayoutMachine`, the pendant spec's §1.2 policy as a pure type: buffer-threshold start (500 ms on desktop), EOM-before-threshold, silent pause on underrun, rebuffer, realtime-paced pull. Chase-play and archive-play are one code path. `PlayoutNormalizer`, the receiver half of CLIENT.md "Audio levels": per-message, silence-gated make-up gain toward −18 dBFS plus a soft limiter, applied to decoded PCM in `renderTick` (the separator tone bypasses it); the sender half is the voice-processing capture path in `AVAudioIO`.
